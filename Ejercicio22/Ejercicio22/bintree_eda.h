...
protected:
    int diametro(Link raiz, int& actual, int &mayor) {
        int diamIz = 0, diamDer =0;
        if (raiz == nullptr) return 0;
        if (raiz->left != nullptr || raiz->right != nullptr) {
            if (raiz->left != nullptr) diamIz = diametro(raiz->left, actual, mayor);
            if (raiz->right != nullptr) diamDer = diametro(raiz->right, actual, mayor);

            if (mayor < diamIz + diamDer + 1) mayor = diamIz + diamDer + 1;
            return actual = std::max(diamIz, diamDer) + 1;
...
template <typename T>
int bintree<T>::diametro() {
    int actual =0, mayor =0;
    diametro(this->raiz, actual, mayor);
    return mayor;
}
