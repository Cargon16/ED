...
protected:
    void menorelem(T &menor, Link raiz) {
        if (raiz->left != nullptr || raiz->right != nullptr) {
            if(raiz->left != nullptr)
            menorelem(menor, raiz->left);

            if(raiz->right != nullptr)
            menorelem(menor, raiz->right);

            if (menor > raiz->elem) menor = raiz->elem;
       }
        else{
            if (menor > raiz->elem) menor = raiz->elem;
        }
...
template <typename T>
T bintree<T>::menorElemento() {
    T menor = this->root();
    menorelem(menor, this->raiz);
