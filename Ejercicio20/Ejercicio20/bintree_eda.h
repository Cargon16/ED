...
protected:
   ...
   //Calcula la frontera del arbol
   void frontera(Link raiz, std::vector<int>& v) {
       if (raiz != nullptr) {
           if (raiz->left == nullptr && raiz->right == nullptr) v.push_back(raiz->elem);
           else {
               frontera(raiz->left, v);
               frontera(raiz->right, v);
           }
       }
   }
