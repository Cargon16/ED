...
protected:
    std::pair<bool, T> lower_b(Link raiz, T const& e) const {
        std::pair<bool, T> p;

        if (raiz == nullptr) return { false, 0 };
        if (raiz->elem == e) return { true, raiz->elem };

        if (menor(e, raiz->elem)) {
            if (raiz->iz != nullptr) {
                p = lower_b(raiz->iz, e);
            }
            else return { true, raiz->elem };
        }
        else
            if (menor(raiz->elem, e)) {
                p = lower_b(raiz->dr, e);
            }
        if (!p.first) {
            if (raiz->elem > e) p= { true, raiz->elem };
        }
