...
protected:
    int rescatadores(Link raiz, int &actual, int &mayor) {
        int resIz = 0, resDer =0;
        int actIz = 0, actDer = 0;
        if (raiz == nullptr) return 0;
        if (raiz->left != nullptr || raiz->right != nullptr) {
            if (raiz->left != nullptr) resIz = rescatadores(raiz->left, actIz, mayor);
            if (raiz->right != nullptr) resDer = rescatadores(raiz->right, actDer, mayor);

            actual = std::max(actIz, actDer) + raiz->elem;
            if (actual > mayor) mayor = actual;
            if (resIz != 0 || resDer != 0) return resIz+resDer;
            if (raiz->elem != 0)
                return 1;
            else return 0;
        }
...
int bintree<T>::rescatadores(int &mayor) {
    int actual = 0;
    mayor = 0;
    return rescatadores(this->raiz, actual, mayor);
}
#endif // bintree_eda_h
