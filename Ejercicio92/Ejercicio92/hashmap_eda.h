...
public:
   ...
   iterator erase(iterator const& it) {
       if (it == this->end())
           throw std::out_of_range("El iterador esta al final");
       else {
           iterator sig = it;
           ++sig;
           
           ListNode* aBorrar = it.act;
           if (array[it.ind] == aBorrar)
               array[it.ind] = aBorrar->sig;
           else {
               ListNode* act = array[it.ind];
               while (act->sig != aBorrar)
                   act = act->sig;
